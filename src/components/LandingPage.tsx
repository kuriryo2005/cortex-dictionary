...
const FEATURES: { title: string; body: string }[] = [
  {
    // 集客の投稿がこの例で人を呼んでいる。LP を開いて同じ例が出てこないと
    // 「話が違う」と感じて離脱する。約束と中身を揃える。
    title: "専門分野の英語に強い",
    body: "stress を引くと「精神的ストレス」ではなく「(工学) 応力」が先に出ます。significant なら「(統計) 有意な」。学術モードは、その分野で実際に使われる意味を優先して並べます。",
  },
  {
    title: "1回引けば、辞書10冊分",
  ...
  {
    // 一括抽出は Pro 限定（api/_lib/plan.ts の extract は無料プランで 0）。
    // 機能一覧に無印で並べると、登録してから使えないと気づくことになる。
    title: "英文を貼るだけで単語帳ができる（Pro）",
    body: "論文や記事をそのまま貼り付けると、あなたがまだ保存していない語だけを抜き出します。読みながら単語を拾う作業がなくなります。Pro プランの機能です。",
  },
  {
    title: "間隔反復で忘れない",
...
const FAQ: { q: string; a: string }[] = [
  ...
  {
    q: "既存の単語帳から移行できますか？",
    a: "JSON からの復元に対応しています（Pro プランの機能です）。書き出しは無料プランでもいつでもできるので、あなたのデータが取り出せなくなることはありません。",
  },
  {
    q: "解約はすぐできますか？",
